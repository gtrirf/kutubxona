...
class Rental(models.Model):
    ...
    return_due_date = models.DateTimeField(default=timezone.now() + timedelta(days=14))
...
class Queue(models.Model):
    ...
    def save(self, *args, **kwargs):
        active_rentals = Rental.objects.filter(book=self.book, is_returned=False).count()
        queue_position = Queue.objects.filter(book=self.book).count()
        total_position = active_rentals + queue_position
        self.estimated_wait_time = timedelta(days=total_position * 14) 
